...
export const Navbar = ({ children, className }: NavbarProps) => {
  ...
  // Scroll event: als je meer dan 40px scrolt, wordt visible true
  React.useEffect(() => {
    const onScroll = () => {
  ...
  // Geef visible prop automatisch door aan NavBody en MobileNav
  const enhancedChildren = React.Children.map(children, (child) => {
    ...
    if (
      ...
      // Voor MobileNav altijd visible=true zodat het altijd zichtbaar is
      const isVisible = child.type === MobileNav ? true : visible;
      return React.cloneElement(child as React.ReactElement<NavBodyProps | MobileNavProps>, { visible: isVisible });
    }
    return child;
  });
  ...
  return (
    <motion.div
      ...
      className={cn(
        ...
        // Mobile nav blijft altijd transparant, geen donkere achtergrond
        className,
      )}
    >
      {children}
